...
+ 資料抽象化(abstraction): 可以定義將介面與實作分離的類別
+ 繼承(inheritance): 可以定義出類別作為模型，以捕捉相似型別之間的關係
+ 動態鏈結(dynamic binding): 能讓開發者使用那些型別的物件，不去在意它們之間差異的細節
...
+ 基礎類別定義一個static member，不管多少類別衍生自一個基礎類別，每個static member都只會存在單一實體，並且可透過基礎或衍生來使用static member。
