...
- Accounting method: 會計師對於低位元的bit從1->0以及0->1，需要對於每一次的變動收取$2元，其中$1用於支付0->1，剩餘的$1("credit")將儲蓄起來供後期使用。對於至於1->0可藉由前期運行儲蓄的$1來支付。換句話說，維持每位元具有$1的儲蓄"credit"可支付下一次1->0的變動，會計師就用這個位元1的存款付帳而不再花錢。因此，會計師總共估計需要收取$2n元，相當於總運行的時間為$$ O(n) $$，而均攤成本(amortized cost)為$$ O(1) $$。
